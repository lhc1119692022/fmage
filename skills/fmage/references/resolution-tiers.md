...
- For `openai-images` and `json-images`, Fmage combines the tier with `aspect` and transport limits, then writes the normalized dimensions actually sent upstream to `requested_size` and `request.size`.
