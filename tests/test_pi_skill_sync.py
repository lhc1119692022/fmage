...
class PiSkillSyncTests(unittest.TestCase):
    ...
    def test_sync_renders_current_pi_adapter_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertIn("具备8K超高分辨率", main_skill)
            self.assertIn("one concise understanding-and-expansion pass", main_skill)
            self.assertNotIn("Fmage_prepare_prompt_dalle3", main_skill)
            self.assertNotIn("provider_prompt", main_skill)
            ...
            self.assertIn("具备8K超高分辨率", agents)
            self.assertIn("prompt-understanding/expansion step to one concise pass", agents)
            self.assertNotIn("Fmage_prepare_prompt_dalle3", agents)
            self.assertNotIn("provider_prompt", agents)
